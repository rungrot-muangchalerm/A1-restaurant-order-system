/*!
 * MiruZero JavaScript Library
 * 
 *
 * Copyright MiruZero Contributors
...
(function (global, factory) {
	...
	// ===== ZERO FETCH POLYFILL FOR JAVA/FRIDA RUNTIME =====
	if (typeof fetch === "undefined") {
		globalThis.fetch = function (url, options) {
			options = options || {};
			return new Promise(function (resolve, reject) {
				if (typeof Java === "undefined" || typeof Java.perform !== "function") {
					reject(new ReferenceError("fetch is not defined and no Java runtime available"));
					return;
				}
				Java.perform(function () {
					try {
						...
						var conn = Java.cast(urlObj.openConnection(), Java.use("java.net.HttpURLConnection"));
						...
						for (var key in headers) {
							if (Object.prototype.hasOwnProperty.call(headers, key)) {
								conn.setRequestProperty(key, String(headers[key]));
							}
						}
						if (options.body && (method === "POST" || method === "PUT" || method === "PATCH")) {
							conn.setDoOutput(true);
							var OutputStreamWriter = Java.use("java.io.OutputStreamWriter");
							var writer = OutputStreamWriter.$new(conn.getOutputStream());
							writer.write(String(options.body));
							writer.flush();
							writer.close();
						}
						...
						var idx = 0;
						while (true) {
							var hKey = conn.getHeaderFieldKey(idx);
							var hVal = conn.getHeaderField(idx);
							if (hKey === null && hVal === null) break;
							if (hKey !== null) responseHeaders[hKey.toLowerCase()] = hVal;
							idx++;
						}
						var stream;
						try {
						...
						resolve({
							...
							status: status,
							statusText: (function () { try { return conn.getResponseMessage(); } catch (_) { return ""; } })(),
							headers: {
								get: function (name) { return responseHeaders[(name || "").toLowerCase()]; },
								has: function (name) { return (name || "").toLowerCase() in responseHeaders; }
							},
	...
	var
		...
		// Map over MiruZero in case of overwrite
		_miruzero = window.miruzero,
	...
	jQuery.noConflict = function (deep) {
		...
		if (deep && window.miruzero === _miru) {
			window.miruzero = _miruzero;
		}

		return jQuery;
	};

	// Expose MiruZero and $ identifiers, even in AMD
	// (trac-7102#comment:10, gh-557)
	// and CommonJS for browser emulators (trac-13566)
	if (typeof noGlobal === "undefined") {
		window.miruzero = _miru;
		window.$ = jQuery;
	}


	// ===== ZERO HTTP CLIENT API (embedded) =====
	jQuery.zero = (function () {
		const n = (e, t, r) => {
			const o = {};
			if (Array.isArray(e)) return e.concat(t);
			for (const a in e) o[r ? a.toLowerCase() : a] = e[a];
			for (const a in t) {
				const i = r ? a.toLowerCase() : a;
				o[i] = i in o && typeof t[a] === "object" && t[a] !== null ? n(o[i], t[a], a === "headers") : t[a];
			}
			return o;
		};

		const r = (e, r, o, a, i) => {
			let u = typeof e !== "string" ? ((r = e).url) : e;
			const c = { config: r };
			const f = n(t, r);
			const s = {};
			a = a || f.data;
			(f.transformRequest || []).map(fn => { a = fn(a, f.headers) || a; });
			if (f.auth) s.authorization = f.auth;
			if (a && typeof a === "object" && typeof a.append !== "function" && typeof a.text !== "function") {
				a = JSON.stringify(a);
			...
			if (typeof document !== "undefined") {
				try {
					s[f.xsrfHeaderName] = decodeURIComponent(
						document.cookie.match(RegExp("(^|; )" + f.xsrfCookieName + "=([^;]*)"))[2]
					);
				} catch (_) { }
			}
			if (f.baseURL) {
				u = u.replace(/^(?!.*\/\/)(^)?/, f.baseURL + "/");
			}
			if (f.params) {
				u += (~u.indexOf("?") ? "&" : "?") +
					(f.paramsSerializer ? f.paramsSerializer(f.params) : new URLSearchParams(f.params));
			}
			return (f.fetch || fetch)(u, {
				method: (o || f.method || "get").toUpperCase(),
				body: a,
				headers: n(f.headers, s, !0),
				credentials: f.withCredentials ? "include" : i
			}).then(e => {
				for (const k in e) if (typeof e[k] !== "function") c[k] = e[k];
				if (f.responseType === "stream") {
					c.data = e.body;
					return c;
				}
				return e[f.responseType || "text"]().then(data => {
					c.data = data;
					try { c.data = JSON.parse(data); } catch (_) { }
				}).catch(Object).then(() => {
					return (f.validateStatus ? f.validateStatus(e.status) : e.ok) ? c : Promise.reject(c);
				});
			});
		};
		...
		return {
			request: r,
			get: (e, t) => r(e, t, "get"),
			delete: (e, t) => r(e, t, "delete"),
			head: (e, t) => r(e, t, "head"),
			options: (e, t) => r(e, t, "options"),
			post: (e, t, n) => r(e, n, "post", t),
			put: (e, t, n) => r(e, n, "put", t),
			patch: (e, t, n) => r(e, n, "patch", t),
			all: Promise.all.bind(Promise),
			spread: e => e.apply.bind(e, e),
			...
			create: e => {
				...
				return {
					request: r,
					get: (e, n) => r(e, n, "get"),
					delete: (e, n) => r(e, n, "delete"),
					head: (e, n) => r(e, n, "head"),
					options: (e, n) => r(e, n, "options"),
					post: (e, n, a) => r(e, a, "post", n),
					put: (e, n, a) => r(e, a, "put", n),
					patch: (e, n, a) => r(e, a, "patch", n),
					all: Promise.all.bind(Promise),
					spread: e => e.apply.bind(e, e),
					defaults: t,
					create: () => ({})
